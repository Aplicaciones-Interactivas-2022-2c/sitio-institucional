...
import { getName, getType, getEmail, getPhone, getUserId } from "../../hooks/authhook";
...
export default function ModifyProfileComponent() {
    ...
    const DatosProfesor = () => {
        ...
        <Grid container spacing={2} sx={{ mt: 1 }}>
          {userData?.map((experiencia, i) => {
                return (<Grid container spacing={2} key={i} item xs={12}>
                <Grid item xs={5}>
                  <TextField required id="outlined-required" label="Titulo" value={experiencia.type} disabled={!editing} onInput={(event) => inputChange(event, i, 'type')}/>
                </Grid>
                <Grid item xs={5}>
                  <TextField required id="outlined-required" label="Experiencia" value={experiencia.years} disabled={!editing} onInput={(event) => inputChange(event, i, 'years')}/>
                </Grid>
                <Grid item xs={2}>
        ...
        <Grid container spacing={2} sx={{ mt: 1 }}>
          ...
          <Grid item xs={12}>
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <Stack spacing={3}>
                <DesktopDatePicker disabled={true} label="Fecha de Nacimiento" inputFormat="DD/MM/YYYY" value={value} onChange={handleChange} renderInput={(params) => <TextField {...params}/>}/>
              </Stack>
              <Stack spacing={3}>
              <PhoneInput inputClass={styles.formControl} disabled={true} placeholder="Numero de telefono" specialLabel="Numero de telefono" country={'ar'} value={getPhone()} onChange={setPhoneValue}/>
              </Stack>
          ...
          {userData?.map((estudio, i) => {
                return (<Grid container spacing={2} key={i} item xs={12}>
                  <Grid item xs={5}>
                    <TextField required id="outlined-required" label="Titulo Obtenido" value={estudio.title} disabled={!editing} onInput={(event) => inputChange(event, i, 'title')}/>
                  </Grid>
                  <Grid item xs={5}>
                    <FormControl disabled={!editing} required fullWidth>
                      <InputLabel>Estado</InputLabel>
                      <Select value={estudio.status} label="Estado" onChange={(event) => inputChange(event, i, 'status')}>
    ...
    return (<div className={styles.ModifyProfileComponent}>
      ...
      <Container component="main" maxWidth="xs">
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate autoComplete="off" sx={{
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              <TextField required id="outlined-required" label="Nombre" defaultValue={getName()} disabled={true}/>
            </Grid>
            <Grid item xs={12}>
              <TextField required id="outlined-required" label="Correo" defaultValue={getEmail()} disabled={true}/>
            </Grid>
          ...
          <Button type="submit" fullWidth variant="contained" sx={{ mt: 3, mb: 2 }} disabled={!editing} onClick={guardarCambios()}>
            Cambiar Datos
          </Button>
        </Box>
